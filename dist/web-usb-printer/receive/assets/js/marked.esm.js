...
var m = {
  ...
  He = h(ce, "gu")
    ...
    .replace(/punct/g, ue)
    .getRegex(),
  Ze = h("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu")
    .replace(/notPunctSpace/g, le)
    .replace(/punctSpace/g, W)
    .replace(/punct/g, D)
    .getRegex(),
  ...
  Fe = h("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>")
    .replace("comment", je)
...
`,
          ...
          g = 0;
        if (
          (this.options.pedantic ? ((g = 2), (c = f.trimStart())) : x ? (g = t[1].length + 1) : ((g = t[2].search(this.rules.other.nonSpaceChar)), (g = g > 4 ? 1 : g), (c = f.slice(g)), (g += t[1].length)),
          x &&
...
`),
        ...
        let T = null,
          Y;
        this.options.gfm && ((T = this.rules.other.listIsTask.exec(c)), T && ((Y = T[0] !== "[ ] "), (c = c.replace(this.rules.other.listReplaceTask, "")))),
          i.items.push({ type: "list_item", raw: p, task: !!T, checked: Y, loose: !1, text: c, tokens: [] }),
          (i.raw += p);
      }
      let a = i.items.at(-1);
...
`)
    ...
    if (n.length === r.length) {
      ...
      for (let o of i) s.rows.push(V(o, s.header.length).map((a, u) => ({ text: a, tokens: this.lexer.inline(a), header: !1, align: s.align[u] })));
      return s;
    }
  }
  lheading(e) {
...
`
  ...
  tag(e) {
    ...
    if (t)
      return (
        !this.lexer.state.inLink && this.rules.other.startATag.test(t[0]) ? (this.lexer.state.inLink = !0) : this.lexer.state.inLink && this.rules.other.endATag.test(t[0]) && (this.lexer.state.inLink = !1),
  ...
  link(e) {
    ...
    if (t) {
      ...
      return (
        ...
        fe(t, { href: r && r.replace(this.rules.inline.anyPunctuation, "$1"), title: i && i.replace(this.rules.inline.anyPunctuation, "$1") }, t[0], this.lexer, this.rules)
      );
    }
  }
  reflink(e, t) {
      ...
      e.loose
        ? e.tokens[0]?.type === "paragraph"
          ? ((e.tokens[0].text = n + " " + e.tokens[0].text),
            e.tokens[0].tokens && e.tokens[0].tokens.length > 0 && e.tokens[0].tokens[0].type === "text" && ((e.tokens[0].tokens[0].text = n + " " + w(e.tokens[0].tokens[0].text)), (e.tokens[0].tokens[0].escaped = !0)))
    ...
    return (n, r) => {
      ...
      if (s.async)
        return Promise.resolve(s.hooks ? s.hooks.preprocess(n) : n)
          ...
          .then((p) => (s.walkTokens ? Promise.all(this.walkTokens(p, s.walkTokens)).then(() => p) : p))
          .then((p) => u(p, s))
