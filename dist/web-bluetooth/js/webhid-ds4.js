 ...
 * Current Interface
 */
var DualShock4Interface;
(function (DualShock4Interface) {
  DualShock4Interface["Disconnected"] = "none";
  /** The controller is connected over USB */
  DualShock4Interface["USB"] = "usb";
  /** The controller is connected over BT */
  DualShock4Interface["Bluetooth"] = "bt";
})(DualShock4Interface || (DualShock4Interface = {}));
/**
...
class DualShock4Rumble {
   ...
   */
  async setRumbleIntensity(light, heavy) {
...
const createBuffer =
  buffer_1.from &&
  buffer_1.alloc &&
  buffer_1.allocUnsafe &&
  buffer_1.allocUnsafeSlow
    ? buffer_1.from
    : // support for Node < 5.10
      (val) => new buffer_1(val);
...
const crc1 = defineCrc("crc1", function (buf, previous) {
  if (!buffer_1.isBuffer(buf)) buf = createBuffer(buf);
...
const crc81wire = defineCrc("dallas-1-wire", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = TABLE$1[(crc ^ byte) & 0xff] & 0xff;
  }
...
const crc16 = defineCrc("crc-16", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = (TABLE$2[(crc ^ byte) & 0xff] ^ (crc >> 8)) & 0xffff;
  }
...
const crc16ccitt = defineCrc("ccitt", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = (TABLE$3[((crc >> 8) ^ byte) & 0xff] ^ (crc << 8)) & 0xffff;
  }
...
const crc16modbus = defineCrc("crc-16-modbus", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = (TABLE$4[(crc ^ byte) & 0xff] ^ (crc >> 8)) & 0xffff;
  }
...
const crc16kermit = defineCrc("kermit", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = (TABLE$5[(crc ^ byte) & 0xff] ^ (crc >> 8)) & 0xffff;
  }
...
const crc24 = defineCrc("crc-24", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = (TABLE$6[((crc >> 16) ^ byte) & 0xff] ^ (crc << 8)) & 0xffffff;
  }
...
const crc32 = defineCrc("crc-32", function (buf, previous) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = TABLE$7[(crc ^ byte) & 0xff] ^ (crc >>> 8);
  }
...
const crcjam = defineCrc("jam", function (buf, previous = -1) {
  ...
  for (let index = 0; index < buf.length; index++) {
    const byte = buf[index];
    crc = TABLE$8[(crc ^ byte) & 0xff] ^ (crc >>> 8);
  }
...
class DualShock4 {
  constructor() {
    ...
    this.lightbar = new DualShock4Lightbar(this);
    /** Allows rumble control */
    this.rumble = new DualShock4Rumble(this);
  ...
  async sendLocalState() {
    ...
    if (this.state.interface === DualShock4Interface.USB) {
      ...
      crcDv.setUint32(0, crc32(buffer_1.from(report.slice(0, 75))));
